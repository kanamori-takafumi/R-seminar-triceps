...
# data generate
beta <- c(rep(1,s),rep(0,d-s)); beta
X <- matrix(rnorm(n*d),n); y <- X %*% beta + rnorm(n,sd=0.01)
...
# plot
par(mfrow=c(1,2)); plot(la$b[1:d,ncol(la$b)],lwd=2,t='h',m="lasso") # lasso
...
par(mfrow=c(1,2)); plot(la$b[1:d,ncol(la$b)],type='h',m="lasso")
plot(el$b[1:d,ncol(el$b)],type='h',m="erastic net")

# plot: num. of selected variables
par(mfrow=c(1,1)); 
plot(colSums(abs(el$b)),colSums(el$b!=0),col=2,type='l',lwd=2,xlab='weakness of regularization',ylab="num: non") # erastic net
lines(colSums(abs(la$b)),colSums(la$b!=0),lwd=2,lty=2) # lasso
lines(cbind(colSums(abs(el$b)),n),lty=3) # num. of sample size
...
x <- diag(length(beta)); y <- as.vector(x %*% beta + rnorm(50,sd=0.2))
