...
data(UN)
par(mfrow=c(1,1))
plot(UN[,4],UN[,7],log='xy', xlab=colnames(UN)[4],ylab=colnames(UN)[7]) # plot data: GDP vs. Infant Mortality

# Least Square Method
logUN <- log(na.omit(UN[,c(4,7)]))          
res <- lm(infantMortality~ppgdp,data=logUN) 
res                                         
y <- predict(res)  # predict

# plot data: GDP vs. Infant Mortality
par(mfrow=c(1,1))
plot(UN[,4],UN[,7],log='xy', xlab=colnames(UN)[4],ylab=colnames(UN)[7])
...
# plot
par(mfrow=c(1,1)); plot(x,y)   
lines(tx,predy,lwd=2,col=2, main='strong regularization')
